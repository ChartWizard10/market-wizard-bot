"""Phase 14S.4 — final-tier dedup reconciliation regression tests.

The SNIPE ladder and downgrade-only seal run after base tiering. Dedup/cooldown
must judge the final post-arbitration tier, not a stale preliminary tier.
"""
...
def _identity_ladder(result, _config):
    return result


def _identity_seal(result, _config):
    return result
...
def test_starter_to_sniper_promotion_beats_recent_starter_cooldown():
    """A real SNIPER_A promotion must not inherit stale STARTER suppression."""
    cfg = _cfg_market_hours()
    state = _state_with_recent_alert("STARTER")
    preliminary = _starter_tiering_result()

    patches = _common_patches(preliminary, _promote_starter_to_sniper, _identity_seal)
    for p in patches:
        p.start()
    try:
...
def test_sealed_down_sniper_does_not_inherit_stale_tier_improvement_permission():
    """A final STARTER must be deduped as STARTER after the seal, not as pre-seal SNIPE."""
    cfg = _cfg_market_hours()
    state = _state_with_recent_alert("STARTER")
    preliminary = deepcopy(_snipe_tiering_result(TICKER))

    patches = _common_patches(preliminary, _identity_ladder, _downgrade_sniper_to_starter)
    for p in patches:
        p.start()
    try:
        summary = _run(run_scan_pipeline(
