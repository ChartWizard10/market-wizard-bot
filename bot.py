...
if df.empty: return {}
c=df["Close"].squeeze();h=df["High"].squeeze();l=df["Low"].squeeze();v=df["Volume"].squeeze();o=df["Open"].squeeze()
...
atr=tr.rolling(14).mean().iloc[-1];vol_r=float(v.iloc[-1]/v.rolling(20).mean().iloc[-1])
...
return dict(ticker=ticker,cur=round(cur,2),h52=round(h52,2),l52=round(l52,2),h20=round(float(h.tail(20).max()),2),l20=round(float(l.tail(20).min()),2),e20=round(float(e20.iloc[-1]),2),e50=round(float(e50.iloc[-1]),2),e200=round(float(e200.iloc[-1]),2),rsi=round(float(rsi.iloc[-1]),1),vol_r=round(vol_r,2),atr=round(float(atr),2),bull=float(e20.iloc[-1])>float(e50.iloc[-1])>float(e200.iloc[-1]),cons=bool(tr.rolling(5).mean().iloc[-1]<tr.rolling(20).mean().iloc[-1]*0.75),f382=round(sh-fr*0.382,2),f618=round(sh-fr*0.618,2),e1618=round(sh+fr*0.618,2),sh=round(sh,2),sl=round(sl,2),fvg=fvg,ob=ob,sweep=sweep,mss=mss,eqhi=round(float(h.tail(10).max()),2),eqlo=round(float(l.tail(10).min()),2),pct=round((cur-h52)/h52*100,1),ohlcv=df.tail(10)[["Open","High","Low","Close","Volume"]].round(2).to_string())
...
@bot.event
async def on_ready(): print("Market Wizard LIVE "+str(bot.user))
...
except Exception as e: await msg.edit(content="Error: "+str(e))
...
try: await ctx.send(wizard(t)[:1900])
except Exception as e: await ctx.send("Error on "+t+": "+str(e))
@bot.command(name="help")
async def help_cmd(ctx): await ctx.send("MARKET WIZARD\n!analyze [TICKER]\n!scan\n!energy\n!help\nWatchlist: "+", ".join(WATCHLIST)+"\nMin 3:1 RR. SNIPE IT=all 6 L7 confirmed.")
